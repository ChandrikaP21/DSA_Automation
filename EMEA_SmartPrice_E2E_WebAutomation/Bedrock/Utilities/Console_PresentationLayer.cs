...
namespace EMEA_SmartPrice_E2E_WebAutomation.Bedrock.Utilities
{
    /*
...
Errors :	
    ...
    class Console_PresentationLayer
    {
        ...
        public static void Report_Run_Progress(string status, bool reportCurrentRunStatus)
        {
            ...
            catch (ExceptionHandlingBlock.ShowStopperException showStopperException)
            {
                throw showStopperException;
            }
            catch (Exception err)
            {
                string customErrorMessage = "Error caught inside Console_Presentatio_Layer.Report_Run_Progress - Failed to Report Run Progress";
                ExceptionHandlingBlock.ShowStopperException showStopperExec = new ExceptionHandlingBlock.ShowStopperException(customErrorMessage, err);
                throw showStopperExec;
            }
            finally
            {
