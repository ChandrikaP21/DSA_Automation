...
namespace EMEA_SmartPrice_E2E_WebAutomation.Quote.CopyNewQuote_Direct.D13
{
    public class ScenarioMain :D01.ScenarioMain
    {
            ...
            try
            {
                ...
                foreach (DSAPageObject item in retriveSmartPriceData)
                {
                    ...
                    if (ds.IsdiscountModified)
                    {
                        ...
                        item.comparisionObjects.CalUnitSellingPrice = Math.Round((100 - Convert.ToDecimal(ds.comparisionObjects.CalDiscountedPricePerUnit)) * Convert.ToDecimal(GenericHelper.ConvertCurrencyIntoString(item.UnitListPrice,item.country)) / 100,2).ToString();
                    }
                     else
                    {
                        item.comparisionObjects.CalUnitSellingPrice = Math.Round((100 - Convert.ToDecimal(item.DiscountInputField)) * Convert.ToDecimal(GenericHelper.ConvertCurrencyIntoString(item.UnitListPrice, item.country)) / 100, 2).ToString();
                    }
                    ...
                    item.comparisionObjects.CalListPrice = Convert.ToDecimal(GenericHelper.ConvertCurrencyIntoString(item.TotalListPrice, item.country));
        ...
        private void PublishReport( DSAPageObject ds,ResultLogging resultLog,TotalFinancialCalculation totalFinancialCalculation,string scenarioId,int testCaseNumber,TestCase test, List<DSAPageObject> retriveSmartPriceDataList,string pricePoint,string runId,bool isIndirect=false,CustomerClass customerClass=null,bool IsStandardPartnerDiscount=false)
        {
            ...
            resultLog.GenerateReportInCSV(ds, retriveSmartPriceDataList, totalFinancialCalculation, scenarioId, testCaseNumber, null,customerClass,true, pricePoint, isIndirect, IsStandardPartnerDiscount);
        ...
        {
            DraftQuotePageObject draftObj = new DraftQuotePageObject(WebDriver);
            try
            {
                Scroll(ds.ByDiscountField(j), ds, j);
                ds.DiscountField(j).Click();
                ds.FloorDiscount = draftObj.DsaFloorDiscount.Text.Replace("%", "");
                ds.CompAnchorDiscount = draftObj.DsaCompAnchorDiscount.Text.Replace("%", "");
            ...
            catch (Exception ex)
            {
                ...
                new ShowStopperException("Unable to modify dicosunt Plese refer  BetweenCompAnchorAndFloor()", ex);
            }
        ...
        public void BelowFloor(DSAPageObject ds,int j)
        {
            ...
            catch (Exception ex)
            {
                ...
                new ShowStopperException("Unable to modify dicosunt Plese refer  BelowFloor()", ex);
            }
        ...
        public void AtCompAnchor(DSAPageObject ds,int j)
        {
            ...
            catch (Exception ex)
            {
                ...
                new ShowStopperException("Unable to modify dicosunt Plese refer  AtCompAnchor()", ex);
        ...
        {
            decimal rcPriceChange = 0;
            try
            {
                DraftQuotePageObject draftObj = new DraftQuotePageObject(WebDriver);
                Scroll(ds.ByDiscountField(j), ds, j);
                ds.DiscountField(j).Click();
                ds.RecommenededDiscount = draftObj.DsaRecommenededDiscount.Text.Replace("%", "");
                ds.CompAnchorDiscount = draftObj.DsaCompAnchorDiscount.Text.Replace("%", "");
            ...
            catch (Exception ex)
            {
                ...
                new ShowStopperException("Unable to modify dicosunt Plese refer  BetweenRecommendedAndCompAnchor()", ex);
            }
        ...
        {
            decimal aboveRecommndedPriceChange = 0;
            try
            {
                DraftQuotePageObject draftObj = new DraftQuotePageObject(WebDriver);
                Scroll(ds.ByDiscountField(j), ds, j);
                ds.DiscountField(j).Click();
                ds.comparisionObjects.CalDiscountedPricePerUnit = draftObj.DsaRecommenededDiscount.Text.Replace("%", "");
                ...
                Thread.Sleep(2000);
                Bedrock.Utilities.Console_PresentationLayer.Report_Run_Details("AboveReommended :" + ds.DiscountField(j).GetAttribute("value"), true);
            ...
            {
                new ShowStopperException("Unble to enter discount abouve recommeneded",ex);
            }
        ...
        public void RetriveDataFromDSAScreenForSpecificPricePoint(TotalFinancialCalculation totalFinancialCalculation, string scenarioId, DSAPageObject ds, string ServiceHeaderText, List<ServicesModule> servicModule, string serviceListPrice,Constant constant,List<DSAPageObject> retriveSmartPriceData,TestCase test)
        {
            ...
            try
            {
                ...
                while (constant.Count < constant.NoOfProducts.Count)
                {
                    ...
                    if (constant.IsElementPresent(constant.ByViewMoreConfiguration(constant.Count)) /*&&isControlFound*/)
                    {
                        WebDriverWait wait = new WebDriverWait(WebDriver, TimeSpan.FromSeconds(StaticBriefCase.TimeOutSecond));

                        wait.Until(ExpectedConditions.ElementToBeClickable(constant.ByViewMoreConfiguration(constant.Count)));
        ...
        public void RetriveDataFromDSAScreen(List<DSAPageObject> retriveSmartPriceDataList,TotalFinancialCalculation totalFinancialCalculation, bool isProduct,DSAPageObject ds, Constant constant, ResultLogging resultLog,int testCaseNumber,TestCase test,string ScenarioId, string ServiceHeaderText = null, List<ServicesModule> servicModule = null, string serviceListPrice = null,bool IsIndirect=false,CustomerClass customerClass=null,PNRAPILog pnrApilog1=null, PNRAPILog pNRAPILog2=null)
        {
            ...
            if (isProduct == true)
            {
              ...
              //      string discountedIndex = index.ToString();
                        ...
                        D13DiscountChange(retriveSmartPriceDataList,resultLog, testCaseNumber, test, GreaterThanR, totalFinancialCalculation, ScenarioId, ds, ServiceHeaderText = null, servicModule = null, serviceListPrice = null, drobj, constant,IsIndirect, customerClass,pnrApilog1,pNRAPILog2);
                        count++;
                    ...
                    D13DiscountChange(retriveSmartPriceDataList,resultLog, testCaseNumber, test, BetweenRAndC, totalFinancialCalculation, ScenarioId, ds, ServiceHeaderText = null, servicModule = null, serviceListPrice = null, drobj, constant, IsIndirect, customerClass, pnrApilog1, pNRAPILog2);
                    EmptyTheList(retriveSmartPriceDataList);
                    BetweenCompAnchorAndFloor(ds,index);
                   customerPage.RefreshSmartPriceButton.Click();
                    Thread.Sleep(4000);
                    
                    D13DiscountChange(retriveSmartPriceDataList,resultLog, testCaseNumber, test, BetweenCAndF, totalFinancialCalculation, ScenarioId, ds, ServiceHeaderText = null, servicModule = null, serviceListPrice = null, drobj, constant, IsIndirect, customerClass,pnrApilog1,pNRAPILog2);
                   ...
                   customerPage.RefreshSmartPriceButton.Click();
                    ...
                    D13DiscountChange(retriveSmartPriceDataList,resultLog, testCaseNumber, test, LessThanFloor, totalFinancialCalculation, ScenarioId, ds, ServiceHeaderText = null, servicModule = null, serviceListPrice = null, drobj, constant, IsIndirect, customerClass,pnrApilog1,pNRAPILog2);
                    EmptyTheList(retriveSmartPriceDataList);
                    AtCompAnchor(ds,index);
                    customerPage.RefreshSmartPriceButton.Click();
                    Thread.Sleep(4000);
                    
                    D13DiscountChange(retriveSmartPriceDataList,resultLog, testCaseNumber, test, AtC, totalFinancialCalculation, ScenarioId, ds, ServiceHeaderText = null, servicModule = null, serviceListPrice = null, drobj, constant, IsIndirect, customerClass);
        ...
        private void EmptyTheList(List<DSAPageObject> retriveSmartPriceDataList)
        {
            ...
            {
                Bedrock.Utilities.SimpleLogger.LogMessage(string.Format("Unale to remove items from the list, Check EmptyTheList()", ex));
            }
        }
        ...
        private void D13DiscountChange(List<DSAPageObject> retriveSmartPriceDataList, ResultLogging resultLog, int testCaseNumber, TestCase test, string priceChangeText, TotalFinancialCalculation totalFinancialCalculation, string scenarioId, DSAPageObject ds, string serviceListPrice, List<ServicesModule> servicModule, string ServiceHeaderText, DraftQuotePageObject drobj, Constant constant,bool isIndirect=false,CustomerClass customerClass=null,PNRAPILog pnrlog1=null,PNRAPILog pnrapilog2=null)
        {
            try
            {

                Thread.Sleep(3000);
                RetriveDataFromDSAScreen(retriveSmartPriceDataList, totalFinancialCalculation, scenarioId, ds, ServiceHeaderText, servicModule, serviceListPrice);
                ...
                foreach (var list in retriveSmartPriceDataList)
                {
                    list.comparisionObjects.CalDiscountedPricePerUnit = ds.comparisionObjects.CalDiscountedPricePerUnit;
                }
                CompareSmartPrice(retriveSmartPriceDataList, totalFinancialCalculation);
                ...
                if (isIndirect)
                    resultLog.GenerateReportInCSVIndirect(ds, retriveSmartPriceDataList, totalFinancialCalculation, scenarioId, testCaseNumber, "", customerClass, isIndirect, "", null, null, false, pnrlog1, pnrapilog2,quoteSummaryObject,goalDetails);
                else
            ...
            {
                Bedrock.Utilities.SimpleLogger.LogMessage(string.Format("UnExpected error occured : Please find D13DiscountChange() in ScenarioMain Class : {0}", ex));
            }
        }
    }
