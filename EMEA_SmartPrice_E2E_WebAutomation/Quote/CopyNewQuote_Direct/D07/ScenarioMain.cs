...
namespace EMEA_SmartPrice_E2E_WebAutomation.Quote.CopyNewQuote_Direct.D07
{
   public  class ScenarioMain:D06.ScenarioMain
    {
       ...
       // Constant cs = new Constant();
        ...
        public void ExecuteTestcases(TestCase test,string scenarioId,int testCaseNumber,string country = null, List<string> application = null)
        {
            ...
            try
                {
                    ...
                    if (!cs.IsElementPresent(cs.ByDraftQuoteNumber))
                    {
                        ...
                        WebDriverUtils.WaitForElementDisplayed(WebDriver, draftQuotePageObject.BydraftquoteNumber, TimeSpan.FromSeconds(3), true);
            ...
            {
                ShowStopperException exec = new ShowStopperException("Exception Occured in D07", e);
                exec.LogException();
        ...
        public void DownGradeServices(Constant locator, TestCase test, DSAPageObject ds)
        {
            try
            {
                ...
                try
                {
                        ...
                        WebDriverUtils.ScrollIntoView(WebDriver, locator.ClickOnServicesWithModuleId(test.ModuleId));
                                    ...
                                    if (locator.IsElementPresent(configPage.ByServiceOptionClickBasedOnListPrice(listPrice.Text)))
                                    {
                                        try
                                        {
                                            ...
                                            if (Convert.ToDecimal(GenericHelper.ConvertCurrencyIntoString(listPrice.Text, ds.country)) < ServiceListPrice)
                                            {
                                                ...
                                                WebDriverUtils.ScrollIntoView(WebDriver, configPage.ByServiceOptionClickBasedOnListPrice(listPrice.Text));
                                                try
                                                {
                                                ...
                                                Bedrock.Utilities.Console_PresentationLayer.Report_Run_Details("Downgrade the service..", true);
                                        ...
                                        catch (Exception ex)
                                        {
                                            ...
                                            Console.WriteLine("There is no list price to downgrade");
                                            throw new ShowStopperException("Unable to downgrade",ex);
            ...
            {
                WebDriverUtils.TakeSnapShot(WebDriver);
                Bedrock.Utilities.Console_PresentationLayer.Report_Run_Errors("Unable to downgrade with module Id ,please refer DownGradeServices()", true);
            }
