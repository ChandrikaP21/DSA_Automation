...
namespace EMEA_SmartPrice_E2E_WebAutomation.DataFactory.DataAccessLayer.TestData
{
    class TestDataReader
    {
        ...
        public static List<string> CountryToTest;
        public static List<string> CountryRegion;
        ...
        public List<Scenario> LoadXmlElements()
        {
            ...
            catch(ShowStopperException showStopperException)
            {
                Bedrock.Utilities.Console_PresentationLayer.Report_Run_Errors(showStopperException.Message,true);
                Bedrock.Utilities.Console_PresentationLayer.Report_Run_Errors(showStopperException.StackTrace, true);
            }
            catch(Exception ex)
            {
                throw;
        ...
        private void LoadXmlConfigurations()
        {
            try
            {
                ...
                XmlNode countryListNode = xmlDoc.SelectSingleNode("DataFeed/Country");
                XmlNode regionListNode = xmlDoc.SelectSingleNode("DataFeed/CountryZone");
                XmlNode maxQuantity = xmlDoc.SelectSingleNode("DataFeed/MaxQuantity");
                XmlNode validityDate = xmlDoc.SelectSingleNode("DataFeed/ValidityDate");
                ApplicationsToTest = applicationsListNode.InnerText.Split(',').ToList<string>();
                CountryToTest = countryListNode.InnerText.Split(',').ToList<string>();
                //CountryRegion = regionListNode.InnerText.Split(',').ToList<string>();
                MaxQuantity = maxQuantity.ToString();
                ValidDate = validityDate.ToString();
                if (CountryToTest.Count < 1 || CountryToTest == null || CountryToTest.Contains(""))
                {
                    //      Bedrock.Utilities.Console_PresentationLayer.Report_Run_Errors("CountryNotfoundException encountered ",true);
            ...
            catch(ShowStopperException showsStopperException)
            {
                showsStopperException.LogException();
                showsStopperException = null;
            }
            catch(Exception err)
            {
        ...
        private void ScenarioD01_LoadTestCase(ref TestCase testCase, XmlNode childnode, string scenarioID)
        {
            try
            {
                ...
                testCase.TestCaseTitle = childnode["TestCaseTitle"].InnerText;//"direct "
                // testCase.BaseQuoteQuery = childnode["BaseQuoteQuery"].InnerText;
                testCase.IsBaseQuoteIdentificationQueryBased = childnode["IsBaseQuoteIdentificationQueryBased"].InnerText;//"N"
                ...
                if (scenarioID.ToLower().Contains("d01"))
                {
