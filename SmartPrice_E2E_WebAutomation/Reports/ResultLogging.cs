  ...
  public class ResultLogging:Constant
    {
        ...
        public void GenerateReportInCSV(DSAPageObject dsaObject,List<DSAPageObject> retriveSmartPriceData,TotalFinancialCalculation totalFinancialCalculation,string scenarioId,int testCaseNumber,string runId, DSAQuoteSummaryObject quoteSummaryObject = null, GoalObject goalDetails = null)
        {


            GenerateReportInCSV(dsaObject, retriveSmartPriceData, totalFinancialCalculation, scenarioId, testCaseNumber, runId,"",null,null,null);
        ...
        public void GenerateReportInCSV(DSAPageObject dsaObject, List<DSAPageObject> retriveSmartPriceData, TotalFinancialCalculation totalFinancialCalculation, string scenarioId, int testCaseNumber,string runId, string refreshID,CustomerClass customerObj, List<PNRAPILog> sp1Log,List<PNRAPILog> sp2Log,bool isindirect=false,bool IsStandardPartnerDiscount=false)
        {
            ...
            string subFolderPath = string.Format("{0}{1}{2}{3}{4}{5}", rootfolder, "\\", "R_"+s+"_"+ scenarioId + "_", runId, "\\", scenarioId + "_" + runId);
        ...
        public void GenerateReportInCSV(DSAPageObject dsaObject, List<DSAPageObject> retriveSmartPriceData, TotalFinancialCalculation totalFinancialCalculation, string scenarioId, int testCaseNumber, string runId, string refreshID, CustomerClass customerObj, bool IsScenarioD13, string pricePointChange,bool isindirect=false,bool IsStandardPartnerDiscount=false)
        {
            if (IsScenarioD13)
            {
                randomNumber = RandomNumber(1000, 10000).ToString();
                ...
                string s = retriveSmartPriceData.Select(x => x.LocalChannelId).First();
                string subFolderPath1 = string.Format("{0}{1}{2}{3}","R_" + s + "_" + scenarioId+"", runId, "\\", scenarioId + "_" + runId);
        ...
        public void GenerateReportInCSVIndirect(DSAPageObject dsaObject, List<DSAPageObject> retriveSmartPriceData, TotalFinancialCalculation totalFinancialCalculation, string scenarioId, int testCaseNumber, string runId, string refreshID, CustomerClass customerObj, bool isIndirect, string pricePointChange,List<PNRAPILog> sp1, List<PNRAPILog> sp2,bool IsIndirect=false,bool IsStandardPartnerDiscount=false)
        {
            if (isIndirect)
            {
                randomNumber = RandomNumber(1000, 10000).ToString();
        ...
        public void CreateCSVForCommonGuidance(StringBuilder sb1,DSAPageObject report,List<PNRAPILog> sp1,List<PNRAPILog> sp2,string scenarioId,DSAPageObject dsaObject,bool IsStandardPartnerDiscount)
        {
          ...
          //  SmartPricePopUpClass spPopUpObject = new SmartPricePopUpClass();
        ...
        public void CreateCSVForAccessoriesLineItemLevel(StringBuilder sb1, DSAPageObject report)
        {
            ...
            //sb1.AppendLine(string.Format("{0}{1}", ",TotalListPrice,", "," + report.TotalListPrice.Replace(",", "") + "," + report.comparisionObjects.CalListPrice + "," + report.comparisionObjects.IsListPriceMatch));
            //sb1.AppendLine(string.Format("{0}{1}", ",TotalSellingPrice,", "," + report.TotalSellingPrice.Replace(",", "") + "," + report.comparisionObjects.CalTotalSellingPrice + "," + report.comparisionObjects.IsTotalSellingPriceMatch));
            //sb1.AppendLine(string.Format("{0}{1}", ",ModifiedRevenue,", "," + report.LineItemSummaryModifiedRevenue.Replace(",", "") + "," + Math.Round(report.CalculatedActualSPCompFinancials.ModifiedRevenue, 2) + "," + report.comparisionObjects.IsModifiedRevenueMatch));
            //sb1.AppendLine(string.Format("{0}{1}", ",SmartPriceRevenue,", "," + report.LineItemSmartRevenue.Replace(",", "") + "," + Math.Round(report.comparisionObjects.TotalActualSmartPriceRevenue, 2) + "," + report.comparisionObjects.IsActualSmartPriceRevenueMatch));
            //sb1.AppendLine(string.Format("{0}{1}", ",Upsell Services Modified Revenue,", "," + report.LineItemSummaryUpsellServicesModifiedRevenue.Replace(",", "") + "," + Math.Round(report.CalculatedActualSPCompFinancials.UpsellModifiedRevenue, 2) + "," + report.comparisionObjects.IsUpSellServicesModifiedRevenueMatch));
            //sb1.AppendLine(string.Format("{0}{1}", ",PricingModifier,", "," + report.LineItemSummeryPricingModifier.Replace(",", "") + "," + Math.Round(report.comparisionObjects.CalculatedTotalPricingModifier, 2) + "," + report.comparisionObjects.IsPricingModifierMatch));
            //sb1.AppendLine(string.Format("{0}{1}", ",With Services Incentive,", "," + report.LineItemSummaryWithServicesIncentive.Replace(",", "") + "," + Math.Round(report.CalculatedActualSPCompFinancials.WithServicesIncentive, 2) + "," + report.comparisionObjects.IsWithServicesIncentiveMatch));
            //sb1.AppendLine(string.Format("{0}{1}", ",WithDFSIncentive,", "," + report.WithDFSIncentive.Replace(",", "") + "," + Math.Round(report.CalculatedActualSPCompFinancials.DFSModifier, 2) + "," + report.comparisionObjects.IsDFSModifierMatch));
            sb1.AppendLine(string.Format("{0}{1}{2}{3}", ",-------------------------------------------", ",-------------------------------------------", ",-------------------------------------------", ", -------------------------------------------"));
        }

        public void ExportToSqlServerDB(DSAPageObject calculateObj,TestCase testCase, string scenarioId,List<DSAPageObject> retriveSmartPriceData)
        {
            try
            {
