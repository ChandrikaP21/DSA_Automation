   ...
   
    {
            ...
            try
            {
                ...
                while (i < viewmore.Count)
                {
                    try
                    {
                        ...
                        if (element.IsElementPresent(element.ByOrderCode(OrderCode)))
                        {
                            if (element.OrderCode(OrderCode).Displayed)
                            {
                                ...
                                WebDriverUtils.ScrollIntoView(WebDriver, configpage.ConfigItem(i));
                ...
                if (isfound == false)
                {
                    Bedrock.Utilities.Console_PresentationLayer.Report_Run_Details("OrderCode is not present,Component is not  added.",true);
                }
            }
            catch (Exception ex)
            {
        ...
        public void ClickOnConfigItem(string orderCode)
        {
            ...
            {   Constant locator = new Constant(WebDriver);
                locator.jse.ExecuteScript("window.scrollBy(0, -1100)");
                Thread.Sleep(2000);
